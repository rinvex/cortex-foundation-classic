<div style="padding-top: 5px;">
    @if($model->exists)
        <small>
            <strong>{{ trans('cortex/foundation::common.route_id') }}:</strong> {{ $model->getRouteKey() }}
        </small>
        @if(request()->user()->can('update', $model->createdBy))
            | <small>
                <strong>{{ trans('cortex/foundation::common.database_id') }}:</strong> {{ $model->getKey() }}
            </small>
        @endif
