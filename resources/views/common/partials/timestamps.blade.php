    ...
    @if($model->exists)
        ...
        @if($model->updated_at)
            <small>
                ...
                @if($model->updatedBy && request()->user()->can('update', $model->updatedBy)) {{ trans('cortex/foundation::common.by') }} <a href="{{ route(request()->accessarea().'.cortex.auth.'.Str::plural($model->updatedBy->getMorphClass()).'.edit', [$model->updatedBy->getMorphClass() => $model->updatedBy]) }}">{{ $model->updatedBy->username }}</a> @endif
            </small>
        @endif
    @endif
</div>
