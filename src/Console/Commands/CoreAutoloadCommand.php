...
class CoreAutoloadCommand extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'cortex:autoload {--f|force : Force the operation to run when in production.}';
    ...
    public function handle(): void
    {
        $commands = collect(Artisan::all())->filter(function ($command) {
            return mb_strpos($command->getName(), 'cortex:autoload:') !== false;
